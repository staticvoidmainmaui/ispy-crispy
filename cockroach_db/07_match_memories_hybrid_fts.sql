--  ─── Hybrid recall  ───  
-- dense + full-text (ts_rank), fused by RRF, that fused signal used as RELEVANCE inside file 04's composite.
--
...
as $$
  ...
  -- tokenize the query once : changed to plainto_tsquery
  q as (
  ...
  -- sparse half: ts_rank -> rank, matches only : changed from ts_rank_cd using proximity weight
  fts_ranked as (
    select
      f.id,
      rank() over (ORDER BY ts_rank(f.content_tsv, q.query) DESC) as fts_rank
      FROM filtered f , q
      WHERE f.content_tsv @@ q.query
  ),
  --ts_rank will happily score non-matches as 0, so the @@ filter is what keeps the sparse list to actual lexical hits.
